...
- **自動実行**: Railwayで毎朝9:15（JST）に自動実行（月〜金）
...
Railwayのcron jobで毎朝9:15（JST）に月〜金で自動実行されます。

### Railway環境変数の設定

Railwayダッシュボードの Variables で以下を設定：
...
`railway.toml` の `cronSchedule` で実行時刻を変更できます：

```toml
cronSchedule = "15 0 * * 1-5"  # UTC 0:15 = JST 9:15, 月〜金
```
